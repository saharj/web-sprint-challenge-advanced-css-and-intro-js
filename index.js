...
function get20s(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    yearsLived = arr[i].years.split(" - ");
    if (yearsLived[0] >= 1900 && yearsLived[1] <= 2000) {
      artistsList.push(arr[i].name);
    }
  }
  return artistsList;
}
...
function lotsOfArt(arr) {
  ...
  const hasManyArts = [];
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].paintings && arr[i].paintings >= 100) {
      hasManyArts.push(arr[i].name);
    }
  }
...
function getHTML(arr) {
  ...
  for (const elem in arr) {
    ...
    imageFile = arr[elem].name.split(" ").join("-");
    htmlToLog = `
      <div id="artist">
        <div class="image">
            <img src="./assets/${imageFile}.jpg"/>
        </div>
        <div class = "name">
          <a href=${arr[elem].wikipedia}>${arr[elem].name}</a>
        </div>
        <div class = "bio">${arr[elem].bio}</div>
      </div>
    `;
...
function randomize(arr) {
  ...
  let copyArr = [...arr];
  const newArr = [];
  const randomIndx = (length) => {
    return Math.floor(Math.random() * length);
  };

  while (copyArr.length > 0) {
    randomI = randomIndx(copyArr.length);
